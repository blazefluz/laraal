...
class AdminController extends Controller
{
    ...
    {
        $user = Auth::user();
        $winners = DB::table('winners')->paginate(20);
        
        return view('admin.winners', compact('winners','user'));
    }
    public function drawpage($game_code)
    {
        $user = Auth::user();
        $game_data =  Lottery::where('code', $game_code)->first();
        if(!$game_data){
            return redirect()->back();
        }
        
        return view('admin.draw', compact('game_data','user'));
    }

    public function random_selector($game_code)
    {
        $user = Auth::user();
        $game_data =  Lottery::where('code', $game_code)->first();
        $validate_draw = DB::table('winners')->where([['game_id', $game_data->id],['status', 1]])->count();
        if($validate_draw >= $game_data->no_winners){
            return redirect()->back()->with('message', ' Sorry the winner has already been selected');
        }else if($validate_draw < $game_data->no_winners){
            $winners = Tickets::orderByRaw('RAND()')->where('game_id',$game_data->id)->limit($game_data->no_winners - $validate_draw)->get();
        }else{

            $winners = Tickets::orderByRaw('RAND()')->where('game_id', $game_data->id)->limit($game_data->no_winners)->get();
        }
        ...
        foreach ($winners as $key => $value ){
            DB::table('winners')->insert([
                ...
                'ticket_id' => $value['ticket_code'],
                'status' => 1,
                'created_at' => Carbon::now()
            ]);
        }
        return view('admin.result', compact('game_data','user','winners'));
    }
    ...
    {
        $user = Auth::user();
        $payments =  DB::table("payments")->join('users', 'users.id', '=', 'payments.user_id')->select(['payments.*','users.first_name','users.last_name', 'users.acc_bal'])->paginate(10);
